...
template <typename Key, typename Value,
        ...
        private:
                ...
                typename list<Element>::iterator findElement(const Key& x, const size_t buck$
...
hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {
        pair<typename hashmap<Key, Value, Compare, Hash>::Element*, bool> theInput;
        ...
        if (it == elems_[bucket].end())
        {
                ...
                theInput.first = &(*(--elems_[bucket].end()));
                theInput.second = true;
                return theInput;
        }
        else
        {
                theInput.first = nullptr;
                theInput.second = false;
                return theInput;
        }
}
...
hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {
        ...
        size_t bucket = hash_.hash(x);
        typename list<pair<const Key, Value>>::iterator theIterator = findElement(x, bucket);
        if (theIterator != elems_[bucket].end())
        {
                --size_;
                if (++theIterator != elems_[bucket].end())
                {
                        theIterator = elems_[bucket].erase(--theIterator);
                        return make_pair(&(*theIterator), true);
                }
                else
                {
                        int theBucket = bucket++;
                        if (theBucket >= hash_.numBuckets()) theBucket = 0;

                        while (elems_[theBucket].empty())
                        {
                                ++theBucket;
                                return make_pair(&(*elems_[theBucket].begin()), true);
                        }
                }

                if (size_ == 0)
                {
                        elems_[bucket].erase(theIterator);
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
      Element* found = find(x);
        if (found == nullptr)
        { // if key not found, create new element with empty value

                auto a = insert(make_pair(x, Value()));
                found = a.first;
        }
...
{
        vector<list<Element>> theVect(n);


        if (n > size_)
        {

                DefaultHash<Key> theDefaultHash(n);

                for (auto firstIterator = elems_.begin(); firstIterator != elems_.end(); ++f$
                        for (auto secondIterator = firstIterator->begin(); secondIterator !=$
                        {
                                size_t bucket = theDefaultHash.hash(secondIterator->first); $
                                theVect[bucket].push_back(*secondIterator);
                        }

                elems_ = std::move(theVect);
        }
